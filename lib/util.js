...
module.exports.formatJsonFiles = () => {
  ...
  tasks.forEach((taskPath) => {
    try {
      ...
      // consistent key order, pls
      const sortedTask = sortJson(task, {depth: 1});

      // two-space indentation
      const formattedTaskJson = JSON.stringify(sortedTask, null, 2);

      // write with a trailing newline
      fs.writeFileSync(taskPath, `${formattedTaskJson}\n`);
    } catch (error) {
...
module.exports.buildDocs = (docsDir = 'docs', options = {}) => {
  ...
  tasks.forEach((taskPath) => {
    try {
      ...
      task.tags
        ? task.tags.forEach(addToTagHash)
        : addToTagHash('Uncategorized');
      ...
      if (task.online_store_javascript) {
        ...
        fs.writeFileSync(
          `${taskDocsDir}/online_store_javascript.js.liquid`,
          task.online_store_javascript.trim() + `\n`,
        );
      }
      ...
      if (task.order_status_javascript) {
        ...
        fs.writeFileSync(
          `${taskDocsDir}/order_status_javascript.js.liquid`,
          task.order_status_javascript.trim() + `\n`,
        );
      }
